...
func (m *implMode3AES) Signer(sk PrivateKey) Signer {
	return mode3aes.NewSigner(sk.(*mode3aes.PrivateKey))
}

func (m *implMode3AES) Verifier(pk PublicKey) Verifier {
	return mode3aes.NewVerifier(pk.(*mode3aes.PublicKey))
}
