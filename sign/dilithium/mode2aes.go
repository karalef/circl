...
func (m *implMode2AES) Signer(sk PrivateKey) Signer {
	return mode2aes.NewSigner(sk.(*mode2aes.PrivateKey))
}

func (m *implMode2AES) Verifier(pk PublicKey) Verifier {
	return mode2aes.NewVerifier(pk.(*mode2aes.PublicKey))
}
